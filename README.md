...
Run the local server to preview the site with hot-reloading:
```bash
npm run dev
```
Open your browser and navigate to the local address displayed in your shell terminal (usually `http://localhost:5173`).

### Production Compilation

Compile and bundle optimized assets for deployment (Vercel, Netlify, or GitHub Pages):
```bash
npm run build
```
The compiled, production-ready website will be generated inside the `dist/` directory.

### Preview Production Build

Preview your compiled assets locally before deploying:
```bash
npm run preview
```
...
## ✍️ Updating Your Profile Info

You can easily modify your skills, contact info, or add new projects inside `src/App.jsx`. All data is stored in neat, documented arrays at the top of the file:

- **Skills**: Update percentages or add skills in the `LANGUAGES` and `FRAMEWORKS_AI` arrays (Lines 240–254).
- **Projects**: Edit descriptions or add GitHub repositories in the `PROJECTS` list (Lines 256–293).
- **Work Timeline**: Update academic and ML research achievements in the `TIMELINE` block (Lines 295–317).
- **Contact Details**: Change your email address in the mail CTA (Line 769).
